 ...
 * 
 * Example: array word input contain special sign, Ex: after preprocser normal
 * input, 'Thu sau' ->_e6 -> DayOfWeek Friday. Note: parse word without space
 * char.
 */
public abstract class ParseStringModel<T> extends AbstractModel {

	public enum ParseStringMode {
		ONE_MAP_ONE, ONE_MAP_MANY
	}
	...
	/***
	 * Create Data from word input (static or dynamic) with mode
	 * {@link ParseStringMode#ONE_MAP_ONE}
	 */
	protected abstract T createData(String word);

	/***
	 * Create Data from word input (static or dynamic) with mode
	 * {@link ParseStringMode#ONE_MAP_MANY} Default return {@link Collection} with
	 * one item using {@link ParseStringModel#createData(String)}
	 */
